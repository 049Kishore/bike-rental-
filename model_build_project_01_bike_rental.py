# -*- coding: utf-8 -*-
"""Model_Build_Project_01-Bike_Rental.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1HvnIE5nbUhZidEoykqmBnXZdJ8bO4pVa
...
ts_df.to_csv('BikeRentalCount.csv',index=False)
...
"""Insights from the PACF Plot:
    ...
    Significant Spike at Lag 1:
        ...
        There’s a strong spike at lag 1, and a rapid drop afterwards.
...
"""### ML Algorithm and Hyperparameter Tunning"""
...
random_grid = {'fit_intercept': fit_intercept,
               'n_jobs': n_jobs,'copy_X':copy_X, 'positive': positive}

lr = LinearRegression()
lr_cv = GridSearchCV(estimator=lr,param_grid = random_grid,n_jobs=-1,verbose=2)
lr_cv.fit(X_train,y_train)
lr_best_param = lr_cv.best_params_
print(f"Best paramters: {lr_best_param})")
...
random_grid = {'n_neighbors':n_neighbors,'weights': weights,
               'n_jobs': n_jobs,'algorithm':algorithm, 'leaf_size': leaf_size}

knn = KNeighborsRegressor()
knn_cv = GridSearchCV(estimator=knn,param_grid = random_grid,n_jobs=-1,verbose=2)
knn_cv.fit(X_train,y_train)
knn_best_param = knn_cv.best_params_
print(f"Best paramters: {knn_best_param})")
...
dtr_cv = GridSearchCV(estimator=dtr,param_grid = random_grid,n_jobs=-1,verbose=2)
dtr_cv.fit(X_train,y_train)
dtr_best_param = dtr_cv.best_params_
print(f"Best paramters: {dtr_best_param})")
...
rf_cv = GridSearchCV(estimator=rfr_1,param_grid = random_grid,n_jobs=-1,verbose=2)
rf_cv.fit(X_train,y_train)
rf_best_param = rf_cv.best_params_
print(f"Best paramters: {rf_best_param})")
...
models = {
    'Linear Regression': LinearRegression(copy_X=True, fit_intercept=True, n_jobs=2, positive=True),
    'KNN Regressor': KNeighborsRegressor(algorithm='ball_tree', leaf_size=5, n_jobs=2, n_neighbors=7, weights='distance'),
    'Decision Tree Regressor': DecisionTreeRegressor(criterion='squared_error', max_depth=10, max_leaf_nodes=7, min_samples_split=2, splitter='best'),
    'Random Forest Regressor': RandomForestRegressor(criterion='poisson', min_samples_leaf=1, min_samples_split=2)
}
...
for name, model in models.items():
    ...
    print(f"Root Mean Absolute Error: {mae:.4f}")
...
new_pred = lr.predict([[1,1,2,8,1,1,0,1,0.59,0.3582,9,111]])

print("Linear Regression Prediction Result:",int(new_pred))

# KNN
knn = KNeighborsRegressor(algorithm='ball_tree', leaf_size=5, n_jobs=2, n_neighbors=7, weights='distance')
knn.fit(X_train,y_train)
new_pred_1 = knn.predict([[3,1,6,2,0,5,1,1,0.79,0.00,0,12]])

print("KNN Prediction Result:",int(new_pred_1))

# Decision Tree
dtr = DecisionTreeRegressor(criterion='squared_error', max_depth=10, max_leaf_nodes=7, min_samples_split=2, splitter='best')
dtr.fit(X_train,y_train)
new_pred_3 = dtr.predict([[3,1,6,2,0,5,1,1,0.79,0.00,0,12]])

print("Decision Tree Prediction Result:",int(new_pred_3))
...
"""From the above models Random Forest Algorithm prediction results are best, when compared to other models."""
